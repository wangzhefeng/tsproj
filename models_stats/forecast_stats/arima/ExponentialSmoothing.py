...
train = y[:"1997-12-01"]
test = y["1998-01-01":]
logger.info(f"Length of train: {len(train)}\nLength of test: {len(test)}")
...
def ses_optimizer(train, alphas, step = 48):
    ...
    for alpha in alphas:
        ...
        logger.info(f"alpha:{round(alpha, 2)}, mae:{round(mae, 4)}")
    logger.info(f"best_alpha:{round(best_alpha, 2)}, best_mae:{round(best_mae, 4)}")
...
def des_optimizer(train, alphas, betas, step = 48):
    ...
    for alpha in alphas:
        for beta in betas:
            ...
            logger.info(f"alpha:{round(alpha, 2)}, beta:{round(beta, 2)}, mae:{round(mae, 4)}")
    logger.info(f"best_alpha:{round(best_alpha, 2)}, best_beta:{round(best_beta, 2)}, best_mae:{round(best_mae, 4)}")
...
def tes_optimizer(train, abg, step = 48):
    ...
    for comb in abg:
        ...
        logger.info([round(comb[0], 2), round(comb[1], 2), round(comb[2], 2), round(mae, 2)])
