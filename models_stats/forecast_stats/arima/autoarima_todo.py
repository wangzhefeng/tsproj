...
class AutoARMA:
    ...
    def Predict(self, pred_len = 20):
        ...
        predict_outside = self.properModel.predict(
            len(predict_inside), 
            len(predict_inside) + pred_len, 
            dynamic = True
        )
    ...
    def Train(self):
        ...
        for p in np.arange(max_p):
            for q in np.arange(max_q):
                # print p, q, self.bic
                model = ARIMA(train_ts, order = (p, q))
                try:
    ...
    def Confirm_p_q(self):
        ...
        AIC = sm.tsa.arma_order_select_ic(
            train_ts, 
            max_ar = p_max, 
            max_ma = q_max, 
            ic = 'aic'
        )['aic_min_order']
        BIC = sm.tsa.arma_order_select_ic(
            train_ts, 
            max_ar = p_max, 
            max_ma = q_max, 
            ic = 'bic'
        )['bic_min_order']
        HQIC = sm.tsa.arma_order_select_ic(
            train_ts, 
            max_ar = p_max, 
            max_ma = q_max, 
            ic = 'hqic'
...
def main():
    ...
    # data
    yv = np.array([
        2800, 2811, 2832, 2850, 2880, 2910, 2960, 3023, 3039, 3056, 
        3138, 3150, 3198, 3100, 3029, 2950, 2989, 3012, 3050, 3142, 
        3252, 3342, 3365, 3385, 3340, 3410, 3443, 3428, 3554, 3615, 
        3646, 3614, 3574, 3635, 3738, 3764, 3788, 3820, 3840, 3875, 
        3900, 3942, 4000, 4021, 4055
    ])
    ...
    model.Train()
    logger.info(model.p, model.q, model.bic, model.diff_count)

    # model predict
    pred_inside, pred_outside = model.Predict()
    logger.info(len(pred_inside), len(yv))
