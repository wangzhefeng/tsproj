...
class Datasplit:
    ...
    def split_data(self):
        ...
        for index in range(len(self.data) - self.timestep):
            ...
            dataY.append(self.data[index + self.timestep][0])
    ...
    def DirectMultiOutput(self):
        ...
        for index in range(len(self.data) - self.timestep - 1):
            dataX.append(self.data[index:(index + self.timestep)])
            dataY.append(self.data[(index + self.timestep):(index + self.timestep + self.output_size)][:, 0].tolist())
        dataX = np.array(dataX)
        dataY = np.array(dataY)
        # 训练集大
        train_size = int(np.round(self.split_ratio * dataX.shape[0]))
    ...
    def RecursiveMultiStep(self):
        ...
        # 将整个窗口的数据保存到 X 中，将未来一个时刻的数据保存到 Y 中
        for index in range(len(self.data) - self.timestep):
            dataX.append(self.data[index:(index + self.timestep)][:, 0])
            dataY.append(self.data[index + self.timestep][0])
        dataX = np.array(dataX)
        dataY = np.array(dataY)
        # 训练集大
...
def main():
    ...
    data_split = Datasplit(data = data.values, timestep = 1, feature_size = 2, split_ratio = 0.8)
    x_train, y_train, x_test, y_test = data_split.split_data()
    logger.info(x_train)
    logger.info(y_train)
    logger.info(x_test)
    logger.info(y_test)
