...
class SmoothAnomalyDetection:
    ...
    def PlotHistory(self, ax, seq_len, is_anomaly, color = "blue", **pltargs):
        # 序列索引
        position_range = np.arange(0, seq_len)
        # 绘制置信区间
        ax.fill_between(
        ...
        # 绘制历史数据点和异常点
        if is_anomaly:
            ax.scatter(seq_len - 1, pltargs["original"][-1], c = "red")
        else:
            ax.scatter(seq_len - 1, pltargs["original"][-1], c = "black")
        # TODO
        ax.scatter(seq_len - 1, pltargs["smooth"][-1], c = color)
    ...
    def PlotRealTimeGif(self):
        ...
        # 实时数据异常值检测
        for seq_len in tqdm(range(self.timesteps + 1), total = (self.timesteps + 1)):
            if seq_len > self.window_len:
                # 使用光滑模型进行异常检测
                if self.method == "convolution":
                    is_anomaly = self.ConvolutionSmoothSeries(seq_len)
                elif self.method == "exponential":
                    is_anomaly = self.ExponentialSmoothSeries(seq_len)
                elif self.method == "decompose":
                    is_anomaly = self.DecomposeSmoothSeries(seq_len)
                elif self.method == "decompose_shift":
                    is_anomaly = self.DecomposeShiftSmoothSeries(seq_len, recovered)
                # 绘图
                for s in range(self.n_series):
                    ...
                    self.PlotHistory(axs[s], seq_len, is_anomaly[s], color = 'blue', **pltargs)
                camera.snap()
            
            if seq_len >= self.timesteps:
                continue
            self.series["original"] = np.hstack([self.series["original"], self.data[:, [seq_len]]])
            if self.method == "decompose_shift":
                self.series["recovered"] = np.hstack([self.series["recovered"], recovered[:, [seq_len]]])
        # 绘制 GIF 图
    ...
    def ConvolutionSmoothSeries(self, seq_len):
        """
        卷积光滑异常值检测
            - 数据没有季节性和趋势
        """
        ...
        if is_anomaly.any():
            self.series["anomaly_id"] = np.hstack([self.series["anomaly_id"], is_anomaly * seq_len]).astype(int)        
        
        return is_anomaly

    def ExponentialSmoothSeries(self, seq_len):
        """
        指数光滑异常值检测
            - 数据有季节性没有趋势
        """
    ...
    def DecomposeSmoothSeries(self, seq_len):
        """
        分解光滑异常值检测
            - 数据有季节性，趋势
        """
    ...
    def DecomposeShiftSmoothSeries(self, seq_len, recovered):
        """
        分解光滑异常值检测
            - 数据有季节性，趋势
        """
        ...
        if is_anomaly.any():
            ...
            self.series["anomaly_id"] = np.hstack([self.series["anomaly_id"], is_anomaly * seq_len]).astype(int)
            recovered[anomaly_series, seq_len] = smoother.smooth_data[anomaly_series, [-1]]
