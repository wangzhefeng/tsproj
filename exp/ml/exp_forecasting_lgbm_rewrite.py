...
class Model:
    ...
    def _split_index(self, window: int):
        """
        数据分割索引构建
        """
    ...
    def _data_split(self, data_X, data_Y, window: int):
        """
        训练、测试数据集分割
        """
        # 数据分割指标
        train_start, train_end, valid_start, valid_end = self._split_index(window)
        # 数据分割
    ...
    @staticmethod
    def _model_evaluate(y_test, y_pred, window: int):
        """
        模型评估
        """
        # 计算模型的性能指标
    ...
    def _process_prediction(self, y_test, y_pred, window: int):
        """
        测试集预测数据
        """
        start_time = self.model_cfgs["time_range"]["start_time"]
    ...
    def cross_validation(self, data_X, data_Y, n_windows: int, drop_last_window: bool = True):
        ...
        for window in range(1, n_windows + 1):
            # 数据分割: 训练集、测试集
            X_train, y_train, \
            X_test, y_test = self._data_split(
                data_X = data_X, 
                data_Y = data_Y, 
                window = window,
            )
            ...
            # 模型评价
            test_scores_df_window = self._model_evaluate(y_test, y_pred, window)
            test_scores_df = pd.concat([test_scores_df, test_scores_df_window], axis = 0)
            # 模型测试集预测数据
            cv_plot_df_window = self._process_prediction(y_test, y_pred, window)
    ...
    def run(self):
        ...
        if self.args.is_test:
            ...
            logger.info(f"model test plot df: \n{plot_df}")
            logger.info(f"model test over...")
        # ------------------------------
        # 模型训练
        # ------------------------------
        if self.args.is_train:
            ...
            logger.info(f"model train over...")
        # ------------------------------
        # TODO 模型预测
        # ------------------------------ 
        if self.args.is_forecast:
            ...
            logger.info(f"model forecast result: \n{y_pred}")
            logger.info(f"model forecast over...")

        return y_pred, test_scores_df, plot_df
