...
def recursive_forecast_backup(model, initial_features, steps):
    """
    递归多步预测
    """
    predictions = []
    current_features = initial_features.copy()
    logger.info(f"start::current_features: {current_features}")

    for _ in range(steps):
        next_pred = model.predict(current_features.reshape(1, -1))
        predictions.append(next_pred[0])
        current_features = np.roll(current_features, shift=-1)
        logger.info(f"1.current_features: {current_features}")
        current_features[-1] = next_pred
        logger.info(f"2.current_features: {current_features}")
        logger.info(f"3.predictions: {predictions}")
...
def main():
    # 生成数据
    df = generate_time_series_data(n_samples=9)
    logger.info(f"df: \n{df}")
    ...
    # 准备训练和测试数据
    X_train, X_test, y_train, y_test = prepare_data(df, train_size=0.6)
    ...
    # 递归多步预测
    initial_features = X_test.iloc[-1].values  # 使用训练集的最后一条样本
