...
def load_data(project: str, data_cfgs: Dict[str, Any] = {}):
    """
    数据加载

    Args:
        project (str): _description_
        model_cfgs (Dict[str, Any], optional): _description_. Defaults to {}.
    ...
    now_date_start_str = data_cfgs["time_range"]["now_time"].strftime("%Y%m%d")
...
def main():
    ...
    future_time = now_time_start + datetime.timedelta(days=predict_days)
    ...
    # result save path
    result_path = f"./saved_results/predict_results/{project}/{node}_dev_{now_time_start.strftime('%Y%m%d')}/"
    ...
    logger.info("=" * 50)
    # load raw data
    input_data = load_data(project=project, data_cfgs=data_cfgs)
    # ------------------------------
    # data preprocessing
    # ------------------------------
    logger.info("=" * 50)
    ...
    logger.info("-" * 40)
    data_history, data_date_history = process_history_data(input_data=input_data, data_cfgs=data_cfgs)
    logger.info(f"Processing future data for training...")
    logger.info("-" * 40)
    data_future, data_date_future = process_future_data(input_data=input_data, data_cfgs=data_cfgs)
    # ------------------------------
    # feature engine
    # ------------------------------
    logger.info("=" * 50)
