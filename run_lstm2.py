...
for epoch in range(num_epochs):
    ...
    for X, Y in tqdm(train_loader):
        ...
        loss.backward()
        optimizer.step()
    logger.info(f"Train Loss: {torch.tensor(train_losses).mean()}")
    # validation
    with torch.no_grad():
        val_losses = []
        ...
        logger.info(f"Valid Loss: {torch.tensor(val_losses).mean()}")
...
logger.info("Test MSE : {:.2f}".format(mean_squared_error(test_preds.detach().numpy().squeeze(), Y_test.detach().numpy())))
logger.info("Test R2 Score : {:.2f}".format(r2_score(test_preds.detach().numpy().squeeze(), Y_test.detach().numpy())))
