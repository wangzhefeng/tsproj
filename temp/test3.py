...
# def pressing_ratio(raw_data):
#     new_min_data = max(raw_data) - ((1 - ratio) * (max(raw_data) + min(raw_data)) / 2)
#     ratio = (max(raw_data) - new_min_data) / (max(raw_data) - min(raw_data))
...
def press_data(raw_data, ratio):
    ...
    # print(f"press_ratio: {press_ratio}")
    for i in raw_data:
...
# press data
# new_df = press_all_data(df=df.loc[(df["time"] >= "2025-09-19 00:00:00") & (df["time"] < "2025-09-20 00:00:00"), :], target_ratios=[0.1, 0.2, 0.3])
...
plt.plot(new_df["time"], new_df["value_10"], label='Cyclic press 10% Component')
plt.plot(new_df["time"], new_df["value_20"], label='Cyclic press 20% Component')
plt.plot(new_df["time"], new_df["value_30"], label='Cyclic press 30% Component')
